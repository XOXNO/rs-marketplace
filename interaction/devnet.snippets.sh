...
deploy() {
    echo ${PROJECT}
    mxpy --verbose contract deploy --metadata-payable-by-sc --arguments 0x64 "erd1cfyadenn4k9wndha0ljhlsdrww9k0jqafqq626hu9zt79urzvzasalgycz" ${SHARD2WrappingWEGLD} str:WEGLD-a28c59 ${ASHSWAP} --bytecode="/Users/truststaking/Documents/GitHub/marketplace/output/xoxno-protocol.wasm" --recall-nonce --ledger --ledger-account-index=0 --ledger-address-index=2 --gas-limit=525000000 --send --proxy=${PROXY} --chain=D || return
...
upgrade() {
    echo "Smart contract address: ${ADDRESS}"
    mxpy contract upgrade ${ADDRESS} --metadata-payable-by-sc --arguments ${ACCUMULATOR} ${ASHSWAP} --bytecode="/Users/mihaieremia/GitHub/marketplace/output/xoxno-protocol.wasm" --recall-nonce --ledger --ledger-account-index=0 --ledger-address-index=2 \
    --gas-limit=350000000 --send --outfile="upgrade.json" --proxy=${PROXY} --chain="D" || return
}


unFreezeAllAuctionIds() {
    mxpy contract call ${ADDRESS} --recall-nonce --ledger --ledger-account-index=0 --ledger-address-index=2 --gas-limit=50000000 --function="unFreezeAllAuctionIds" --send --proxy=${PROXY} --chain=D
}
